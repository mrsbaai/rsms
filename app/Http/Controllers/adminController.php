...
class adminController extends Controller
{
    ...
    public function isDemoNeedUpdate(){
        ...
        $count_free = number::where('info', '<>','no')->where('network', 'textnow')->where('network_login', 'not like', 'aa@%')->where('email', null)->where('is_private', true)->where('last_checked', '>', Carbon::now()->subDays(5)->toDateTimeString())->count();
