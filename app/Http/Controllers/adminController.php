...
class adminController extends Controller
{
    ...
    public function isDemoNeedUpdate(){
        ...
        $count_free = number::where('info', null)->where('network_login', 'not like', 'aa@%')->where('email', null)->where('is_private', true)->where('last_checked', '>', Carbon::now()->subDays(6)->toDateTimeString())->get();
        ...
        foreach ($demoNumbers as $demoNumber) {
            ...
            if ($demoNumber['last_checked'] < Carbon::now()->subMinutes(330)){
                ...
                if ($count_free > 1){
                    ...
                    $newNumber = number::all()->where('network_login', 'not like', 'aa@%')->where('is_private',true)->where('email', null)->where('info', '<>', 'no')->sortBydesc('last_checked')->first();
