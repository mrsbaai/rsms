...
class userController extends Controller
{
    ...
    public function numbers(){
        if (Auth::check()){
            ...
            $avalableNumbers = number::all()->where('is_private',true)->where('is_active',true)->where('email', null)->where('network_login', 'like', '%' . '@' . '%')->where('last_checked', '>', Carbon::now()->subDays(40)->toDateTimeString());
