...
class userController extends Controller
{
    ...
    public function numbers(){
        if (Auth::check()){
            ...
            $avalableNumbers = number::all()->where('is_private',true)->where('active',true)->where('info', "<>", "no")->where('email', null);
    ...
    public function doAddNumbers($amount){
        ...
        if ($price <= $balance){
            $email = Auth::user()->email;
            $numbers = number::all()->where('is_private',true)->where('is_active',true)->where('email', null)->where('info', "<>", "no")->sortBydesc('last_checked')->take($amount);
